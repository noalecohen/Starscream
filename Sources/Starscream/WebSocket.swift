...
open class WebSocket: WebSocketClient, EngineDelegate {
    ...
    public convenience init(request: URLRequest, configuration: URLSessionConfiguration? = nil, certPinner: CertificatePinning? = FoundationSecurity(), compressionHandler: CompressionHandler? = nil, useCustomEngine: Bool = true) {
        if #available(macOS 10.15, iOS 13.0, watchOS 6.0, tvOS 13.0, *), !useCustomEngine {
                debugPrint("ios 13+")
            self.init(request: request, engine: NativeEngine(configuration: configuration))
